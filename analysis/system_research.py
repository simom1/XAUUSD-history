...
SHORT_FAMILY = {k: SHORT_FAMILY_EXT[k] for k in
                ("aroon_up_25", "aroon_down_25", "minus_di_14", "close_vs_ema200", "rsi_14")}
WINDOWS = (2016, 6048)
THRESHOLDS = (1.5, 2.0)
HOLDS = (12, 84)
...
EXITS = (
    ...
    # Catastrophic stops: rarely bind, only cap extreme adverse excursions
    # (gap risk / runaway move) while leaving signal/hold exits in charge.
    ("stop5", {"stop_loss_atr": 5.0}),
    ("stop6", {"stop_loss_atr": 6.0}),
    ("stop8", {"stop_loss_atr": 8.0}),
    ("stop12", {"stop_loss_atr": 12.0}),
    ("stop16", {"stop_loss_atr": 16.0}),
...
CATASTROPHIC_STOPS = ("stop5", "stop6", "stop8")
...
def components(side: str) -> list[Component]:
    family = LONG_FAMILY if side == "long" else SHORT_FAMILY
    return [Component(side, factor, direction, w, t, h, regime, session)
            for factor, direction in family.items() for w in WINDOWS for t in THRESHOLDS
            for h in HOLDS for regime in REGIMES for session in SESSIONS]
...
def session_mask(ts: np.ndarray, name: str) -> np.ndarray:
    if name == "all":
        return np.ones(len(ts), dtype=bool)
    close_min = ((ts + 300) % 86400) // 60
...
def decisions(df: pd.DataFrame, component: Component, factors: pd.DataFrame | None = None,
              gates: dict[str, np.ndarray] | None = None) -> np.ndarray:
    ...
    return base & regime_mask(df, component.side, component.regime, gates) & session_mask(
        df["timestamp"].to_numpy(np.int64), component.session)
...
def system_target(df: pd.DataFrame, long: Component | None, short: Component | None,
                  ...
                  oz: float = OZ) -> tuple[np.ndarray, int]:
    ld = decisions(df, long, factors, gates) if long else np.zeros(len(df), dtype=bool)
    sd = decisions(df, short, factors, gates) if short else np.zeros(len(df), dtype=bool)
    return unified_targets(ld, sd, long.hold if long else 1, short.hold if short else 1, oz)


def bar_metrics(result, lo: int = 0, hi: int | None = None) -> dict:
...
def run_engine(df: pd.DataFrame, long: Component | None, short: Component | None,
    ...
    result = BacktestEngine(BacktestConfig(max_position_oz=oz, **exit_args)).run(
