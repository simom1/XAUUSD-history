"""15m mirror of scripts/run_stability_attribution.py.

Explain single-account walk-forward instability on 15m bars without reading
development data.  Timeframe scaling mirrors run_single_account_research_15m:

- bar_seconds 900, Sharpe annualized with 96 bars/day.
- component ladder W672/W2016 (the 15m analogs of the 5m W2016/W6048),
  holds H4/H28 (the analogs of H12/H84).
- trade gates scaled for 15m frequency: 12 per training prefix (5m: 20),
  40 total test trades across folds (5m: 100).

Each fold selects long/short components and an exit on its training prefix
only, in three shapes (long-only, short-only, long-short).  All selection
uses the research slice; the consumed development segment is never read.
"""
...
from analysis.system_research import EXITS, Component, bar_metrics, components, fast_component_score, run_engine

ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data" / "xauusd_15m_indicators.csv.gz"
REPORT = ROOT / "report" / "single_account_stability_attribution_15m.md"
CSV = ROOT / "report" / "single_account_stability_picks_15m.csv"
SHAPES = ("long_only", "short_only", "long_short")

BAR_SECONDS = 900
ANN = float(np.sqrt(96 * 252))
WINDOWS_15M = (672, 2016)
HOLDS_15M = (4, 28)
MIN_TRAIN_TRADES = 12
MIN_CONSENSUS_TRADES = 40
...
def top(df, factors, gates, side: str) -> list[Component]:
    scored = [fast_component_score(df, c, factors, gates, bar_seconds=BAR_SECONDS, ann=ANN)
              for c in components(side, windows=WINDOWS_15M, holds=HOLDS_15M)]
    valid = [x for x in scored if x["trades_proxy"] >= MIN_TRAIN_TRADES and x["avg"] > 0]
...
def verify_no_exit_calibration(df: pd.DataFrame, target: np.ndarray, result) -> None:
    ...
    session = Session(df.timestamp.to_numpy(np.int64), bar_seconds=BAR_SECONDS)
...
def main() -> None:
        ...
        for shape in SHAPES:
            ...
            for long, short in specs(long_top, short_top, shape):
                for exit_name, _ in EXITS:
                    result, target, metrics, conflicts = run_engine(train, long, short, exit_name,
                                                                    f_train, g_train,
            ...
            result, target, _, test_conflicts = run_engine(context, long, short, exit_name,
                                                           f_context, g_context,
                                                           bar_seconds=BAR_SECONDS, ann=ANN)
            test_m = bar_metrics(result, lo, hi, ann=ANN)
    ...
    lines = ["# Single-account stability attribution (15m bars)\n",
    ...
    if out.empty:
        ...
        eligible = consensus[(consensus.selections >= 3) & (consensus.positive_folds >= 3) &
                             (consensus.trades >= MIN_CONSENSUS_TRADES) & (consensus.avg_trade > 0)]
